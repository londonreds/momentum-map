...
var Event = function ($) {
  return function (properties) {
    ...
    this.render_group = function (distance, zipcode) {
      ...
      if (that.props.url) {
        social_html += '<a href=\'' + that.props.url + '\' target=\'_blank\'><img src=\'/img/icon/globe.png\' /></a>';
      }
      ...
      if (that.props.social) {
        ...
        if (that.props.social.email !== '') {
          social_html += '<a target=\'_top\' href=\'mailto:' + that.props.social.email + '\' ><img src=\'/img/icon/mailchimp.png\' /></a>';
        }
    ...
    this.render_event = function (distance, zipcode) {
      ...
      var rendered = $("<div class=montserrat/>").addClass('event-item ' + that.className).html('\n            <div class="event-item lato ' + that.className + '" lat="' + lat + '" lon="' + lon + '">\n              <h5 class="time-info">\n                <span class="time-info-dist">' + (distance ? distance + "mi&nbsp;&nbsp;" : "") + '</span>' + datetime + '\n              </h5>\n              <h3>\n                <a target="_blank" href="' + that.props.url + '">' + that.props.title + '</a>\n              </h3>\n              <span class="label-icon"></span>\n              <h5 class="event-type">' + that.props.event_type + '</h5>\n              <p>We are working on creating a momentum group here, please sign up, <a href="mailto:groups@peoplesmomentum.com?subject=I%20would%20like%20to%20join%20the%20upcoming%20Momentum%20group%20in%20'+ that.props.address + '&body=Hi%20My%20name%20is%20%7B%7BYOUR_NAME%7D%7D%20and%20I%20would%20like%20to%20join%20the%20group%20in%20' + that.props.address + '">send us an email</a> to join</p>\n              <div>\n                <a class="rsvp-link" href="mailto:groups@peoplesmomentum.com?subject=I%20would%20like%20to%20join%20the%20upcoming%20Momentum%20group%20in%20'+ that.props.address + '&body=Hi%20My%20name%20is%20%7B%7BYOUR_NAME%7D%7D%20and%20I%20would%20like%20to%20join%20the%20group%20in%20' + that.props.address + '" target="_blank">JOIN</a>\n              </div>\n            </div>\n            ');
  ...
  return function (eventData, campaignOffices, zipcodes, options) {
    ...
    var initialize = function initialize() {
      ...
      uniqueLocs.forEach(function (item) {
        ...
        //     .addTo(overlays);
        // }
        if (item.className == 'upcoming-group') {
          L.circleMarker(item.latLng, { radius: 6, className: item.className, color: 'white', fillColor: '#16d300', opacity: 0.8, fillOpacity: 0.7, weight: 2 }).on('click', function (e) {
            _popupEvents(e);
          }).addTo(overlays);
    ...
    */
    ...
    module.filter = function (zipcode, distance, sort, filterTypes) {
      ...
      var _zipcode = zipcode.toUpperCase().replace(/[\s\-]/g, '');
      var targetZipcode = zipcodes[_zipcode];

      console.log(targetZipcode, zipcodes);
      //Remove list
      d3.select("#event-list").selectAll("li").remove();

      if (targetZipcode == undefined || !targetZipcode) {
        $("#event-list").append("<li class='error lato'>Postcode does not exist.</li>");
        return;
      }
